...
def check_diag(n,act_player,move_player) :
    ...
    if n == 1 :
        ...
        if act_cell + empty_cell == FSIZE and empty_cell == 1 : 
            field[empty_line_idx][empty_line_idx] = act_player
            return True
    else        
        for line_idx in range(FSIZE) :              #проверка диагонали №2 атакуем
            ...
            if act_cell + empty_cell == FSIZE and empty_cell == 1 : 
                field[empty_line_idx][FSIZE-1-empty_line_idx] = act_player
                return True
    return False
...
# ход Х
# атака МОЯ                           check_attack_move(X,X)
# атака ПРОТИВНИКА == защита МОЯ      check_attack_move(0,X)
...
def make_comp_move( act_player ) :
    ...
    #       4) если свободны углы - поставить в произвольный свободный угол
    while True :                           #сократил кусок выше с помощью функции randrange. Было 12 строк стало 6. Однако по какой то причине второй while который ставит рандомное значение подсвечен как мертвый код
        comp_line_idx = randint(0,1)*( FSIZE-1 )      #randrange (0,3,2)           
        comp_colomn_idx = randint(0,1)*( FSIZE-1 )    #randrange (0,3,2)
...
def check_winner( act_player ) :
    '''
    + проверка 3-х строк
    + проверка 3-х колонок
    + проверка 2-х диагоналей
    '''
    # перебери каждую строку
    #     внутри строки перебери каждую ячейку
    for line in field : 
        cell_count = 0
        for cell in line : 
    ...
    # перебери каждую колонку
    #     внутри колонки перебрать каждую ячейку
...
#----------------------------------------------  ОСНОВНОЙ КОД. отсюда стартует программа
while True :
